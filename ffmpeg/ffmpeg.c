...
{
    int i, ret;
    loge("ffmpeg_execute 1");
    BenchmarkTimeStamps ti;
    loge("ffmpeg_execute 2");
    init_dynload();
    loge("ffmpeg_execute 3");
    register_exit(ffmpeg_cleanup);
    loge("ffmpeg_execute 4");
    setvbuf(stderr,NULL,_IONBF,0); /* win32 runtime needs this */
    loge("ffmpeg_execute 5");
    av_log_set_flags(AV_LOG_SKIP_REPEATED);
    loge("ffmpeg_execute 6");
    parse_loglevel(argc, argv, options);
    loge("ffmpeg_execute 7");
    ...
    loge("ffmpeg_execute 8");
    /* parse options and open all input/output files */
    ret = ffmpeg_parse_options(argc, argv);
    if (ret < 0) {
        loge("parse or excute options failed!");
        return 1;
    }
    loge("ffmpeg_execute 9");
    if (nb_output_files <= 0 && nb_input_files == 0) {
        show_usage();
        av_log(NULL, AV_LOG_WARNING, "Use -h to get full help or, even better, run 'man %s'\n", program_name);
        return 1;
    }
    loge("ffmpeg_execute 10");
    /* file converter / grab */
    if (nb_output_files <= 0) {
        av_log(NULL, AV_LOG_FATAL, "At least one output file must be specified\n");
        return 1;
    }
    loge("ffmpeg_execute 11");
    for (i = 0; i < nb_output_files; i++) {
        if (strcmp(output_files[i]->ctx->oformat->name, "rtp"))
            want_sdp = 0;
    }
    loge("ffmpeg_execute 12");
